...
class M_Manager_Cards:
    ...
    def create_cards(self):
        #création des cartes
        self.__created_cards.add_card(M_Joker.M_Joker(self.__rules.M_Rules.get_color()[0]))
        self.__created_cards.add_card(M_Joker.M_Joker(self.__rules.M_Rules.get_color()[0]))
        for k in range (len(self.__rules.M_Rules.get_max_Cards_per_player()+1)):
                self.__created_cards.add_card(M_As.M_As(self.__rules.M_Rules.get_color()[0]))
                self.__created_cards.add_card(M_King.M_King(self.__rules.M_Rules.get_color()[0]))
                self.__created_cards.add_card(M_Queen.M_Queen(self.__rules.M_Rules.get_color()[0])) 
    ...
    def draft_cards(self) -> list[M_CardsSet]: 
        ...
        for i in range (self.__rules.M_Rules.get_nb_players()):
            for k in range (self.__rules.M_Rules.get_max_Cards_per_player()):
