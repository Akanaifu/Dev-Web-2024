...
app.post('/data', (req, res) => {
    ...
    if (!tableName || !columns || !values) {
        return res.status(400).json({ message: 'Table name, columns, and values are required' });
    }
    ...
    if (columnsArray.length !== valuesArray.length) {
        return res.status(400).json({ message: 'The number of columns and values must match' });
    }

    const query = `INSERT INTO ${tableName} (${columnsArray.join(',')}) VALUES (${valuesArray.map(() => '?').join(',')})`;
    ...
    db.query(query, valuesArray, (err, results) => {
        ...
        res.json({ success: true, message: 'Données injectées avec succès', id: results.insertId });
    });
});
